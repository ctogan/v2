$(document).ready(function() {
    $("#btn_submit_city").on("click", function() {
        bootbox.confirm({
            title: "Save Data ?",
            centerVertical:true,
    ...
    $('#form_new_city').on('submit', function(event){
        event.preventDefault();

        var btn = $("#btn_submit_city");
        btn.attr("disabled", "disabled");

        var token = $('meta[name="csrf-token"]').attr('content');

        $.ajax({
            url:'/admin/master/city/submit',
            method:"POST",
            headers: {
                'X-CSRF-TOKEN': token
            },
            ...
            success:function(response)
            {
                ...
                if(res.status) {
                    ...
                    bootbox.alert({
                        ...
                        onHide: function(e) {
                            window.location = "/admin/master/city"
                        }
                    });
                }else{
                    close_loading();
    ...
    $('#form_update_city').on('submit', function(event){
        event.preventDefault();

        var btn = $("#btn_update_city");
        btn.attr("disabled", "disabled");

        var token = $('meta[name="csrf-token"]').attr('content');

        $.ajax({
            url:'/admin/master/city/update',
            method:"POST",
            headers: {
                'X-CSRF-TOKEN': token
            },
            ...
            columnDefs: [
                ...
                },
                {
                    ...
                    render: function(data, type, full, meta) {
                        return '<a href="/admin/master/city/edit/'+data+'" class="btn btn-datatable btn-icon btn-transparent-dark btn-sm p-0 mr-2"><i data-feather="edit"></i></a>' +
